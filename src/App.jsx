...
const initData = () => {
  try {
    const saved = localStorage.getItem("flash_data");
    if (saved) {
...
const initTotal = () => {
  try { return parseInt(localStorage.getItem("flash_totalBatches")) || 47; } catch (e) { return 47; }
};
...
export default function App() {
  ...
  const [editable, setEditable] = useState(() => {
    try { return localStorage.getItem("flash_editable") === "true"; } catch (e) { return false; }
  });
  ...
  const tryUnlock = () => {
    if (pwValue === EDIT_PASSWORD) {
      setEditable(true);
      try { localStorage.setItem("flash_editable", "true"); } catch (e) {}
  ...
  const lockEdit = () => {
    setEditable(false);
    try { localStorage.setItem("flash_editable", "false"); } catch (e) {}
  };
  ...
  const saveTotalBatches = (n) => { if (!editable) return;
    ...
    try { localStorage.setItem("flash_totalBatches", String(n)); } catch (e) {} dbSet("flash/total", n);
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: S.bg, color: S.text, fontFamily: "'Pretendard','Apple SD Gothic Neo','Noto Sans KR',sans-serif", padding: "20px 16px" }}>
      ...
      <div style={{ textAlign: "center", marginBottom: 20 }}>
        <h1 style={{ fontSize: 28, fontWeight: 900, margin: 0, letterSpacing: "0.08em", background: "linear-gradient(135deg,#ea580c,#dc2626)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Flash Sorter</h1>
      ...
      {showPaste && (
        <div style={{ background: S.card, border: "1px solid #e9d5ff", borderRadius: 14, padding: 16, marginBottom: 16, boxShadow: S.shadow }}>
          ...
          <textarea value={pasteText} onChange={e => setPasteText(e.target.value)}
            placeholder={"카카오톡 채팅 내용 복사해서 붙여넣기\n예) W존 8차 6배치 불출대기\n    메자닌하부Flash Sorter* 6.5/8차-3배치 대기중"}
      ...
      {/* Grand Total */}
      <div style={{ background: "linear-gradient(135deg,#ea580c,#dc2626)", borderRadius: 16, padding: "20px 24px", marginBottom: 20, display: "flex", alignItems: "center", gap: 20, boxShadow: "0 4px 20px rgba(37,99,235,0.3)" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10, marginBottom: 20 }}>
        {ZONES.map(z => {
          ...
          const isPicking = (data[z]||{done:"",picking:false}).picking;
      ...
      <div style={{ background: S.card, border: `1px solid ${S.border}`, borderRadius: 16, padding: 16, boxShadow: S.shadow }}>
        ...
        {(() => {
          ...
          const bulDone = ZONES.filter(z => zoneTotals[z].pct === 100 && !(data[z]||{done:"",picking:false}).picking);
          const pickDone = ZONES.filter(z => (data[z]||{done:"",picking:false}).picking);
          const inProgress = ZONES.filter(z => !(data[z]||{done:"",picking:false}).picking && zoneTotals[z].pct < 100 && zoneTotals[z].done > 0);
          const notStarted = ZONES.filter(z => !(data[z]||{done:"",picking:false}).picking && zoneTotals[z].pct === 0);
          const lines = [
            `Flash Sorter (${timeStr})`,
          ...
          return (
            <>
              ...
              <button onClick={() => { navigator.clipboard.writeText(text).then(() => setCopied(true)); setTimeout(() => setCopied(false), 2000); }}
                style={{ width: "100%", background: copied ? "#059669" : "linear-gradient(135deg,#ea580c,#dc2626)", border: "none", borderRadius: 8, padding: "10px 0", cursor: "pointer", color: "#fff", fontSize: 13, fontWeight: 700, marginBottom: 14, boxShadow: "0 2px 8px rgba(37,99,235,0.25)" }}>
                {copied ? "✓ 복사됨!" : "📤 현황 공유"}
              </button>
            </>
          );
